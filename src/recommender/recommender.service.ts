...
export class RecommenderService {
  ...
  async getRecommendations(userId: string): Promise<any[]> {
    ...
    try {
      const query = `
        MATCH (u:User {id: $userId})-[:PURCHASED|:ADDED_TO_CART]->(p:Product)
        MATCH (other:User)-[:PURCHASED|:ADDED_TO_CART]->(p)
        MATCH (other)-[:PURCHASED|:ADDED_TO_CART]->(rec:Product)
        WHERE NOT (u)-[:PURCHASED|:ADDED_TO_CART]->(rec)
